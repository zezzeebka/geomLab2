...
void RenderUI(int &_normalType, std::function<void()> buttonCallback, std::function<void(char* buffer)> saveImage, char buffer[256]) {
    ImGui::Begin("Menu");
...
int main() {
    ...
    while (window.isOpen()) {
        ...
        RenderUI(_normalType, buttonCallback, saveImage, buffer);
